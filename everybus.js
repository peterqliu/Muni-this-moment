...
function current_location(lat, lon) {
    if ($('#iamhere').length == 0) {
      d3.select('#markers').append('circle').attr('cx', lonx(lon)).attr('cy', laty(lat)).attr('id', 'iamhere').on('mouseout', function() {
        removelabel()
      });
      d3.select('#findme')
    ...
    d3.select('#iamhere').attr('cx', lonx(lon)).attr('cy', laty(lat)).attr('r', 4).on('mouseover', function() {
...
function newBus(bus, firebaseId) {
    ...
    newmarker.attr('style', '-webkit-transform:translate(' + xcoord + 'px,' + ycoord + 'px)').attr('class', 'busmarker ' + 'route' + bus.routeTag).attr('id', 'bus' + firebaseId).attr('xcoord', xcoord + 20).attr('ycoord', ycoord - 10).attr('type', bus.vtype).attr('route', bus.routeTag).attr('direction', direction).on('mouseover', function() {
      ...
      }).append('g').attr('class', 'grower').append('path').attr('transform', 'rotate(' + bus.heading + ')').attr('d', marker_path)
      //Bus marker text	
    d3.select('#bus' + firebaseId + ' .grower').append('text')
      ...
      }).attr('class', 'busmarkertext').text(bus.routeTag);
...
f.on("child_changed", function(s) {
  ...
  //existing bus
    ...
    if (direction != d3.select('#bus' + busMarker).attr('direction')) {
      d3.select('#bus' + busMarker).attr('direction', direction).attr('route', route).on('click', null).on('mouseover', null).on('mouseleave', null).on('click', function() {
        var xcoordforlabel = d3.select(this).attr('xcoord');
        var ycoordforlabel = d3.select(this).attr('ycoord');
        zoomto(xcoordforlabel, ycoordforlabel);
        showbusesonline(bus.routeTag, direction);
    ...
    $('#bus' + busMarker).attr('style', '-webkit-transform:translate(' + xcoord + 'px,' + ycoord + 'px)').attr('xcoord', xcoord + 20).attr('ycoord', ycoord - 10).select('path').attr('transform', 'rotate(' + s.val().heading + ')').attr('onclick', '')
    countbuses();
  }
});
...
f.on("child_removed", function(s) {
  ...
  if (typeof busMarker !== 'undefined') {
    ...
    delete buses[s.name()];
    d3.select('#bus' + busMarker).remove();
  }
});
...
function zoomto(x, y) {
  ...
  dragreset = 'yes';
  $('#viewport').attr('style', '-webkit-transform-origin:' + (1.5 * x - 500) + 'px ' + (1.7 * y - 400) + 'px; ');
  $()
  setTimeout(function() {
...
function drawroute(line, color) {
    d3.json('stops.refactored.v3.json', function(json) {
      ...
      d3.select('#busstops').selectAll('path').data(data).enter().append("path").attr('d', function(d, i) {
        return data[i]
      }).attr('stroke-width', 3).attr('class', 'routepatha').attr('fill', 'none').attr('stroke', 'purple');
    })
  }
...
function drawpath(routedir, marker) {
    ...
    d3.json('stops.refactored.v3.json', function(json) {
      ...
      d3.select('#markers').append("path").attr('d', pathdata).attr('class', 'routepath').attr('fill', 'none').attr('stroke', directionColor).attr('stroke-linecap', 'round').attr('stroke-linejoin', 'round');
      setTimeout(function() {
      ...
      for (k = 0; k < 2; k++) {
        d3.select('#markers').append("circle").attr('cx', endcaps[k * 2]).attr('cy', endcaps[(k * 2) + 1]).attr('r', 4).attr('id', 'endcap').attr('class', 'routepath coast')
          //.attr('fill', 'none')
...
function showroutestops(routedir, color, vtype) {
    ...
    d3.json('stops.refactored.v3.json', function(json) {
      var data = json[routedir]['Stops'];
      d3.select('#busstops').selectAll("circle").data(data).enter().append("circle").attr("r", 0).attr('class', 'stopmarker').attr('tag', function(d, i) {
        return data[i]['Tag']
...
function zoombackout() {
    ...
    d3.select('.viewport').attr('class', 'viewport');
...
function makelabel(x, y, text, textcolor, subtext, directiontag) {
  ...
  //Fire only when the mouse isn't already held down (i.e. dragging)
  if ($('body:active').length == 0) {
    d3.select('#busstoplabels').attr('transform', 'translate(' + (x + 8) + ' ' + (y - 10) + ') ' + labelscalefactor).append('text').attr('x', 6).attr('y', 13).attr('font-size', 8).attr('fill', '#666').text(text).attr('font-weight', '500').attr('id', 'stoplabel').attr('class', 'heading');
    d3.select('#busstoplabels').append('text').attr('x', 6).attr('y', 22).attr('font-size', 6).text(subtext).attr('id', 'stoplabel').attr('class', 'subtext').append('tspan').attr('class', 'directionspecifier').attr('fill', textcolor).attr('font-weight', '400').text(directiontag).append('tspan').text(' line').attr('class', 'subtext');
    //create background rectangle for label
    d3.select('#busstoplabels').insert('rect', 'text').attr('width', function() {
      return Math.max($('#stoplabel')[0].getComputedTextLength(), $('.subtext')[0].getComputedTextLength()) + 12
    }).attr('height', 28).attr('stroke', '#eee').attr('stroke-width', 0.5).attr('id', 'labelrect');
  }
}
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      if (predictionlist[0] > 0) {
        d3.select('.subtext').text('Next ' + heading + ' ' + vehicle(vtype) + ' in ').append('tspan').attr('fill', directionColor).attr('font-weight', '400').text(timeconverter(predictionlist[0]));
      }
