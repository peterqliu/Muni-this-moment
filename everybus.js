//Automatically reload page if no bus markers in 4 seconds
	setTimeout(function () {console.log('pinging');
		if(document.getElementsByClassName('busmarker').length==0) {location.reload(true)}; 
...
////// PREVIOUSLY IN INDEX.HTML
	...
	function lookup() {
			...
			if (number.length!=0){
				if (targetbuses.length==0) {$('#searchreply').html('No results'); console.log('nonereturned')}
		...
		function getprediction(routedir, stoptag, vtype) {
			...
			function vehicle(input) {if (input=='train'){return 'train'} else {return 'bus'}};
			...
			$.ajax({
		      	...
		      	success: function(response) {		  
					...
					else {$('.subtext').append(' (no current prediction)')}
